...
import { DeviceType } from '../../components/FindPrinter';
...
export interface SelectedPrinter extends Partial<IUSBPrinter & IBLEPrinter & INetPrinter> {
...
export const PrintPage = ({ route, navigation }: any) => {
  ...
  const [selectedNetPrinter, setSelectedNetPrinter] = React.useState<DeviceType>({
  ...
  const handlePrint = async () => {
    ...
    try {
      const Printer = printerList[selectedValue];
      Printer.printText('<C>Printer berhasil tersambung</C>', {
        cut: false,
      });
      // Printer.printText(`${CENTER}${BOLD_ON} Toko Ibnu Ali ${BOLD_OFF}\n`);
      // Printer.printText(`${CENTER} Sembungsemi, Bawang ${OFF_CENTER}`);
      ...
      >
        {devices !== undefined &&
          ...
          devices?.map((item: any, index) => (
            <Picker.Item label={item.device_name} value={item} key={`printer-item-${index}`} />
          ))}
      </Picker>
    </>
  );
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.section}>
        ...
        >
          <TouchableOpacity style={styles.button} onPress={handleConnectSelectedPrinter}>
        ...
        <View style={styles.buttonContainer}>
          ...
          >
            ...
            <Text style={styles.text}>Test Print</Text>
          </TouchableOpacity>
        </View>
        {/* Button Print bill */}
        {/* <View style={styles.buttonContainer}>
