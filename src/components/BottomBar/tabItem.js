...
const TabItem = ({ isFocused, onLongPress, onPress, label }) => {
    ...
    const IconTab = () => {
        ...
        if (label === 'LaporanPage') {
            return <Image source={isFocused ? Dolar : DolarOutline} resizeMode="cover" style={{width: 30, height: 30, tintColor: COLORS.primary }} />
        }
        if (label === 'ProfilePage') {
            return <Icon name={isFocused ? "person" : "person-outline"} size={30} color={COLORS.primary} />;
        }
